...
- **通知已读水位服务端化**：新增 `notification_watermarks` 表（迁移 0005）+ `GET/PUT /notifications/watermark`（水位只前推不回退），前端已读状态跨设备/清缓存不再丢失
- **评论功能（P5）**：`comments` 表 + REST 端点 + MCP `add_comment`/`list_comments`/`delete_comment`（工具数 35 → 38）+ 任务弹窗评论面板；修复依赖移除从 UI 一直失败（query/body 传参不匹配）
- **登录时序侧信道缓解**：用户不存在也执行等价 bcrypt 校验；全局搜索 total 精确化（按实体 count）
- **测试**：120 → 160 后端用例（跨用户隔离、登录限流、依赖环检测、评论、通知水位、搜索），前端 39 → 45
